...
export async function readFile(
  ...
  try {
    ...
    try {
      ...
      // Attempt to auto-hydrate using brctl (macOS iCloud command-line tool)
      let hydrationStarted = false
      try {
        await execFileAsync('brctl', ['download', filePath])
        hydrationStarted = true
      } catch {
        // brctl unavailable or failed — fall through to manual instructions
      }
      throw new Error(
        hydrationStarted
          ? `File is evicted by iCloud. A download has been requested — please try again in a few seconds.`
