...
export function createServer(dirs: MarkdownDirs): Server {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      {
        name: 'save_file',
        description: `Use this tool to save any substantial AI response, analysis, or notes to the user's markdown notes in iCloud. When the user asks to save something, pick a path and call this tool immediately — do NOT call search_files or list_directories first, do NOT ask for confirmation. Folder structure: always use exactly 2 levels deep — top-level topic folder + subtopic folder, never put files directly in a top-level folder. Examples: job-search/udemy/2026-03-24-seo-audit.md, job-search/paypal/2026-03-24-interview-prep.md, travel/baltimore/2026-03-24-trip-notes.md, travel/new-york/2026-03-24-restaurants.md, cooking/italian/2026-03-24-pasta-tips.md. Naming: YYYY-MM-DD-descriptive-slug.md (lowercase kebab-case, max 5 words, today's date prefix). Folders are created automatically. Modes: "create" (default, fails if file exists), "overwrite" (replaces), "append" (adds content under a date heading).`,
        inputSchema: {
          type: 'object' as const,
          properties: {
